...
final class Users extends Base
{
    ...
    public function details($request, $response, $args)
    {
        ...
        if ($id !== null) {
            $users = DB::queryOne(
                'SELECT * FROM users WHERE id = :id AND excluido = false',
                ['id' => $id]
            );
            $contacts = DB::query(
    ...
    public function update($request, $response)
    {
        ...
        try {
            ...
            return $this->json($response, ['status' => true, 'msg' => 'Usuário atualizado com sucesso!', 'id' => (int) $id], 200);
        } catch (Exception $e) {
    ...
    public function delete($request, $response)
    {
        ...
        try {
            DB::execute(
                'UPDATE users SET excluido = true, atualizado_em = :now WHERE id = :id',
                ['now' => (new DateTime())->format('Y-m-d H:i:s'), 'id' => $id]
            );

            return $this->json($response, ['status' => true, 'msg' => 'Usuário removido com sucesso!', 'id' => (int) $id]);
        } catch (Exception $e) {
    ...
    public function listingdata($request, $response)
    {
        ...
        try {
            ...
            $totalRecords    = (int) DB::queryOne('SELECT COUNT(*) as total FROM users WHERE excluido = false')['total'];
            $filteredRecords = (int) DB::queryOne("SELECT COUNT(*) as total FROM users {$where}", $params)['total'];

            $params['limit']  = $length;
            $params['offset'] = $start;

            $users = DB::query(
                "SELECT * FROM users {$where} ORDER BY {$orderField} {$orderType} LIMIT :limit OFFSET :offset",
                $params
            );
