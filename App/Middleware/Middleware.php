...
class Middleware
{
    ...
    # Metodo de autenticação das rotas GET e fluxos de telas públicas
    public static function web()
    {
        $middleware = function ($request, $handler) {
            ...
            # Captura o path atual da URL
            $path = $request->getUri()->getPath();

            # Define se a rota atual faz parte do grupo público (Login e Cadastro)
            $isPublicRoute = ($path === '/login' || $path === '/cadastro');
            ...
            # Já autenticado tentando ver telas públicas (login/cadastro) → manda direto para a home.
            if ($isPublicRoute && $auth) {
                return (new Response())->withHeader('Location', '/home')->withStatus(302);
            }
            ...
            # Caso permitido: autenticado em rota privada ou anônimo nas rotas públicas.
            return $handler->handle($request);
        };
        return $middleware;
    }
