    ...
    public function details($request, $response, $args)
    {
        ...
        return $this->getTwig()
            ->render($response, $this->setView('sale'), [
                ...
                'sale'          => $sale,
                'items'         => $items,
                'customers'     => DB::query('SELECT id, nome, cpf_cnpj FROM customers WHERE excluido = false AND ativo = true ORDER BY nome ASC'),
                'sellers'       => DB::query('SELECT id, nome, sobrenome FROM users WHERE ativo = true ORDER BY nome ASC'),
                'serviceOrders' => DB::query("SELECT id, numero FROM service_orders WHERE excluido = false AND status != 'cancelada' ORDER BY numero DESC"),
            ])
            ->withHeader('Content-Type', 'text/html')
            ->withStatus(200);
    }
