...
async function listAddresses() {
    ...
    const requests = new Requests();
    try {
        const response = await requests.post(`/fornecedor/${Id.value}/enderecoslista`);
...
async function deleteAddress(addressId) {
    ...
    const requests = new Requests();
    try {
        const response = await requests.setForm('form').post(`/fornecedor/endereco/${addressId}`);
        if (!response.status) {
            Swal.fire({
                icon: 'error',
                title: 'Erro',
...
async function listContact() {
    const requests = new Requests();
    try {
        const response = await requests.post(`/fornecedor/${Id.value}/contatoslista`);
...
async function deleteContact(contactId) {
    ...
    const requests = new Requests();
    try {
        const response = await requests.setForm('form').post(`/fornecedor/contato/${contactId}`);
        if (!response.status) {
            Swal.fire({
                icon: 'error',
                title: 'Erro',
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const cpfCnpjInput = document.getElementById('cnpj'); // Certifique-se de que o id no HTML seja este
    if (cpfCnpjInput) {
        Inputmask({
